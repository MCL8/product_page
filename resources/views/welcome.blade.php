    ...
    <body class="antialiased">
        <nav class="navbar">
            <div class="container justify-content-start">
                @if($currency == 'rub')
                    <a class="mr-4" href="{{ route('index', ['currency' => 'kzt']) }}">KZT</a>
                    <b><span>RUB</span></b>
                @else
            ...
            <div class="container">
                <div class="row">
                    ...
                    @foreach($products as $product)
                        <div class="col-4 m-12 mt-8">
                            <img src="/images/{{ $product->image }}" alt="" style="height: 200px">
                            <h4>{{ $product->name }}</h4>
                            <h5>Категория: {{ $product->category->name }}</h5>
                            <b>Артикул: {{ $product->number }}</b>
                            <h5>Цена: {{ $product->formattedPrice($currency, $rate) }}</h5>
                            <b>
                            ...
                            @else
                                @if(\Illuminate\Support\Facades\Auth::check() && $product->in_stock != 0)
                                    <button type="button" class="btn btn-success btn-lg buy-btn"
                                            data-id="{{ $product->id }}"
                                            data-price="{{ $product->getPrice($rate) }}"
